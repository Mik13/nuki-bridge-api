 ...
 * @param {Number}    [options.maxDelayBetweenRequests]   max delay in ms between requests made to the bridge (default=5000)
 * @returns {Bridge}
 * @constructor
 */
var Bridge = function Bridge (ip, port, token, options) {
  ...
  this.maxDelayBetweenRequests = options && options.maxDelayBetweenRequests || 5000;
  ...
  return new Promise(function requestWithoutDelay (resolve, reject) {
    self.delayer = self.delayer
        .then(function doRequest () {
          ...
          req.then(resolve, reject); // Resolve returned promise without delay

          return req;
        })
        .timeout(self.maxDelayBetweenRequests)
        .then(function doNotKeepResult () {}, function ignoreErrorsForNextRequest () {})

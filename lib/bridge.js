...
var Bridge = function Bridge (ip, port, token, options) {
  ...
  this.forcePlainToken = options && options.forcePlainToken || null;
...
Bridge.prototype._request = function _request (action, additionalParameter, forcePlainToken) {
  ...
  return new Promise(resolve => {
    this.delayer = this.delayer
      .then(() => {
        var url = 'http://' + this.ip + ':' + this.port + '/' + action + '?1';

        var tokenParams = this._getTokenParams(this.forcePlainToken || forcePlainToken || false);
      ...
      .delay(this.delayBetweenRequests);
  });
};
  ...
  return this
    ...
    .then(function (nukis) {
      if (!Array.isArray(nukis)) {
        throw new Error('Did not receive a list of nukis! Response: ' + JSON.stringify(nukis));
      }
...
Bridge.prototype.get = function get (nukiId) {
  ...
  return this
    ...
    .then(function validateNuki (nukis) {
      ...
      const nuki = nukis.find(nuki => nuki.nukiId === nukiId);

      if (!nuki) {
        throw new Error('did not find nuki!');
      }

      return new Nuki(self, nukiId, nuki.deviceType);
    });
};
...
Bridge.prototype.addCallbackUrl = function addCallbackUrl(url, listen) {
  var self = this;
  return this
...
Bridge.prototype.addCallback = function addCallback (hostname, port, listen, path) {
  ...
  return this.addCallbackUrl(url, listen)
};

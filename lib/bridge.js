...
var Bridge = function Bridge (ip, port, token) {
  ...
  this.info().then(function(res)
  {
	  self.type = res.bridgeType; // 1 = hardware, 2 = software
  });
};
 ...
 * @private
 */
Bridge.prototype._getTokenParams = function _getTokenParams () {
...
Bridge.prototype._request = function _request (action, additionalParameter) {
  ...
  var tokenParams = this._getTokenParams();
...
Bridge.prototype.get = function get (nukiId, doNotValidate) {
  ...
  if (doNotValidate) {
    return Promise.resolve(new Nuki(self, nukiId));
  }

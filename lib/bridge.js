 ...
 * @param {Number}    [options.requestTimeout]            timeout of requests in ms - should be longer than the nuki needs to (un-)lock
 * @returns {Bridge}
 * @constructor
 */
var Bridge = function Bridge (ip, port, token, options) {
  ...
  this.requestTimeout = options && options.requestTimeout || 10000;
...
/**
 * This function sends a request to the bridge.
 *
 * @param {String}    url     the url
 * @returns {Promise<Object>}
 * @private
 */
Bridge.prototype._requestWithDelayer = function _requestWithDelayer (url) {
  var self = this;

  return new Promise(function requestWithoutDelay (resolve, reject) {
    self.delayer = self.delayer
        ...
        .then(function doNotKeepResult (result) {
          resolve(result);
        }, function ignoreErrorsForNextRequest (error) {
          reject(error);
        })
        .delay(self.delayBetweenRequests);
  }).timeout(self.requestTimeout + self.delayBetweenRequests + 500);
};
...
Bridge.prototype._request = function _request (action, additionalParameter) {
  ...
  return this._requestWithDelayer(url);
};
